...
## [0.1.1] - 2026-01-20
### Changes
- Added `EnemyController` instead of simple iterator function to spawn enemies
- Added `enemies_config.json` as config file for `EnemyController`
- Added atlas variables in `Main.gd` to assign tileset generation
- Removed hand made tileset assignment blobs in `Main.tscn`
- Added `generate_floors` and `generate_walls` func
- Added `generate_map` that calls the above and does simple randomisation based on hard coded min and max width and height
